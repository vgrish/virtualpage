<?php

$_lang['vp_menu_create'] = 'Создать';
$_lang['vp_menu_add'] = 'Добавить';
$_lang['vp_menu_update'] = 'Изменить';
$_lang['vp_menu_remove'] = 'Удалить';
$_lang['vp_menu_active'] = 'Активировать';

$_lang['vp_menu_remove_confirm'] = 'Вы уверены, что хотите удалить эту запись?';

// buttons
$_lang['vp_btn_create'] = 'Создать';
$_lang['vp_btn_save'] = 'Сохранить';
$_lang['vp_btn_edit'] = 'Изменить';
$_lang['vp_btn_view'] = 'Просмотр';
$_lang['vp_btn_delete'] = 'Удалить';
$_lang['vp_btn_cancel'] = 'Отмена';

//
$_lang['vp_actions'] = 'Действия';
$_lang['vp_combo_select'] = 'Нажмите для выбора';

$_lang['vp_id'] = 'Id';
$_lang['vp_name'] = 'Имя';
$_lang['vp_color'] = 'Цвет';

$_lang['vp_active'] = 'Включен';
$_lang['vp_description'] = 'Описание';
$_lang['vp_num'] = 'Номер';
$_lang['vp_status'] = 'Статус';
$_lang['vp_count'] = 'Количество';

$_lang['vp_all'] = 'Все';
$_lang['vp_empty'] = 'Нет';

$_lang['vp_createdon'] = 'Дата создания';
$_lang['vp_updatedon'] = 'Дата изменения';
$_lang['vp_timestamp'] = 'Метка времени';

$_lang['vp_resource'] = 'Ресурс';
$_lang['vp_comment'] = 'Комментарий';
$_lang['vp_options'] = 'Опции';

